...
""".trimIndent()
    ...
    private fun execSql(dbPath: String, sql: String, key: String): String {
        ...
        try {
            val out = RootGateways.runQuiet("$SQLCIPHER '$dbPath' < '${sqlFile.absolutePath}' 2>/dev/null | tail -1").trim()
            return out
    ...
    fun mergeDatabases(
        ...
        if (baseDbPath != outputPath) {
            RootGateways.run("cp '$baseDbPath' '$outputPath'")
        }
...
SELECT * FROM message;
""".trimIndent())
            RootGateways.run("$SQLCIPHER '$overlayDbPath' < '${dumpSqlFile.absolutePath}' 2>/dev/null")
            dumpSqlFile.delete()
            ...
            val changes = try {
                val out = RootGateways.runQuiet("$SQLCIPHER '$outputPath' < '${runSqlFile.absolutePath}' 2>/dev/null | tail -1").trim()
                out.toLongOrNull() ?: 0L
